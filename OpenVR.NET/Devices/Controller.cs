...
public class Controller : VrDevice
{
    ...
    public void UpdateInput()
    {
        ...
        if (_model?.ComponentsLoaded == true)
        {
            componentStates ??= new();
            ...
            foreach (var i in _model.Components)
            {
                if (!Valve.VR.OpenVR.RenderModels.GetComponentStateForDevicePath(i.ParentName, i.Name, Handle, ref state, ref cState))
                    continue;

                Matrix4x4 local = cState.mTrackingToComponentLocal.ToMatrix4x4();
                Matrix4x4 render = cState.mTrackingToComponentRenderModel.ToMatrix4x4();
                Matrix4x4.Decompose(local, out _, out Quaternion localRotation, out Vector3 localPosition);
                Matrix4x4.Decompose(render, out _, out Quaternion renderRotation, out Vector3 renderPosition);

                comp.ScrollWheelVisible = state.bScrollWheelVisible;
                comp.LocalPosition = localPosition;
                comp.LocalRotation = localRotation;
                comp.Position = renderPosition;
                comp.Rotation = renderRotation;
    ...
    public ComponentState? GetComponentState(ComponentModel component)
        => componentStates?.TryGetValue(component, out var state) != true ? null : state;
