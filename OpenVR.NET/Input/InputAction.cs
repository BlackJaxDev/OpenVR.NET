...
{
    /// <inheritdoc cref="Devices.VrDevice.DeviceToAbsoluteTrackingMatrix"/>
...
public class PoseAction : Action
{
    ...
    private PoseInput? FetchData(bool predictNextFrame = false, float predictOffset = 0)
    {
        InputPoseActionData_t data = default;
        var error = predictNextFrame 
            ? Valve.VR.OpenVR.Input.GetPoseActionDataForNextFrame(
        ...
        if (error is EVRInputError.None && data.bActive)
        {
            Matrix4x4 matrix = data.pose.mDeviceToAbsoluteTracking.ToMatrix4x4();
            Matrix4x4.Decompose(matrix, out _, out Quaternion rotation, out Vector3 translation);
            return new()
            {
                DeviceToAbsoluteTrackingMatrix = matrix,
